...
public final class VisualStudioNETProjectWriter implements ProjectWriter {
	...
	private static final String PC_DATA = "#PCDATA";
  ...
  private static void addAttribute(final AttributesImpl attributes, final String attrName, final String attrValue) {
    ...
    if (attrValue != null) {
      ...
      attributes.addAttribute(null, attrName, attrName, PC_DATA, attrValue);
   ...
   */
  private String getAdditionalIncludeDirectories(final CommandLineCompilerConfiguration compilerConfig) {
	  
    final StringBuilder includeDirs = new StringBuilder();
   ...
   */
  private void writeCompilerElement(final ContentHandler content, final boolean isDebug,
    ...
    addAttribute(attributes, "AdditionalIncludeDirectories", getAdditionalIncludeDirectories(compilerConfig));
  ...
  private void writeFilteredSources(final String name, final String filter, final String basePath,
    ...
    filterAttrs.addAttribute(null, "Name", "Name", PC_DATA, name);
    
    filterAttrs.addAttribute(null, FILTER, FILTER, PC_DATA, filter);
    
    content.startElement(null, FILTER, FILTER, filterAttrs);

    final AttributesImpl fileAttrs = new AttributesImpl();
    
    fileAttrs.addAttribute(null, "RelativePath", "RelativePath", PC_DATA, "");
    ...
    for (final File sortedSource : sortedSources) {
      ...
      if (isGroupMember(filter, sortedSource)) {
		...
		try {
			...
			relativePath = CUtil.getRelativePath(basePath, sortedSource);
  ...
  public void writeProject(final File fileName, 
    ...
    writeCompilerElement(content, true, compilerConfig);
    
    writeLinkerElement(content, true, linkTarget);
    ...
    writeCompilerElement(content, false, compilerConfig);
    
    writeLinkerElement(content, false, linkTarget);
