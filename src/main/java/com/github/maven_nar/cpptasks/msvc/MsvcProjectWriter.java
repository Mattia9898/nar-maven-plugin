...
public final class MsvcProjectWriter implements ProjectWriter {
  ...
  private static final String NDEBUG = "NDEBUG";
  ...
  private static String toProjectName(final String name) {
    ...
    final StringBuilder projectNameBuf = new StringBuilder(name);
  ...
  private void writeConfig(final Writer writer, final boolean isDebug) throws IOException {
    ...
    writer.write("# ADD BASE RSC /l 0x409 /d \"" + configMacro + SLASH_R);
    
    writer.write("# ADD RSC /l 0x409 /d \"" + configMacro + SLASH_R);
  ...
  private void writeLinkOptions(final Writer writer, final boolean isDebug,
    ...
    final StringBuilder baseOptions = new StringBuilder(100);
    
    final StringBuilder options = new StringBuilder(100);
  ...
  private void writeMessage(final Writer writer, final String projectName) throws IOException {
  ...
  public void writeProject(final File fileName, final CCTask task, final ProjectDef projectDef, final List<File> files,
    ...
    try(Writer writer = new BufferedWriter(new FileWriter(dspFile));){
    	...
    	writeMessage(writer, projectName);
    	...
    	writer.write("# PROP Scc_ProjName " + SLASH_R);
    	
    	writer.write("# PROP Scc_LocalPath " + SLASH_R);
    	...
    	if (this.version.equals("6.00")) {
    		...
    		writer.write(PROP_DEFAULT_FILTER + sourceFilter + SLASH_R);

    		writer.write(END_GROUP);

    		writer.write("# Begin Group \"Header Files\"\r\n\r\n");
    		
    		writer.write(PROP_DEFAULT_FILTER + headerFilter + SLASH_R);
    		
    		writer.write(END_GROUP);
    		
    		writer.write("# Begin Group \"Resource Files\"\r\n\r\n");
    		
    		writer.write(PROP_DEFAULT_FILTER + resourceFilter + SLASH_R);
