...
public final class TargetHistoryTable {
  ...
  private class TargetHistoryTableHandler extends DefaultHandler {
    ...
    private final HashMap<String, TargetHistory> history;
    
    private String output;
    
    private long outputLastModified;
    
    private final ArrayList<SourceHistory> sources = new ArrayList<>();
     ...
     */
    private TargetHistoryTableHandler(final HashMap<String, TargetHistory> history2) {
   ...
   */
  private final HashMap<String, TargetHistory> history = new HashMap<>(20);
 ...
 * @throws SAXNotRecognizedException 
   */
  public TargetHistoryTable(final CCTask task, final File outputDir) throws BuildException, SAXNotRecognizedException, SAXNotSupportedException, ParserConfigurationException
  {
    ...
    if (this.historyFile.exists()) {
      ...
      try {
        ...
        parser.parse(this.historyFile, new TargetHistoryTableHandler(this.history));
  ...
  public void commit(){
    ...
    if (this.dirty) {
      ...
      //
      final HashMap<String, String> configs = new HashMap<>(20);
      
      Enumeration<TargetHistory> elements = (Enumeration<TargetHistory>) this.history.entrySet();
  ...
  public void update(final ProcessorConfiguration config, final String[] sources, final VersionInfo versionInfo) throws IOException {
  ...
  // FREEHEP added synchronized
  private synchronized void update(final String configId, final String outputName, final String[] sources) throws IOException {
  ...
  // FREEHEP added synchronized
  public synchronized void update(final TargetInfo linkTarget) throws IOException {
