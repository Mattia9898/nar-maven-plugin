...
public abstract class PrecompilingCommandLineCompiler extends CommandLineCompiler implements PrecompilingCompiler {
  ...
  protected PrecompilingCommandLineCompiler(final String command, 
		  									final String[] sourceExtensions,
		  									final boolean libtool, 
		  									final PrecompilingCommandLineCompiler libtoolCompiler, 
		  									final Environment env) {
	  
    super(command, sourceExtensions, libtool, libtoolCompiler,
        env);
  ...
  public CompilerConfiguration[] createPrecompileConfigurations(final CompilerConfiguration config,
    ...
    //
    final CommandLineCompilerConfiguration cmdLineConfig = (CommandLineCompilerConfiguration) config;
    ...
    if (includes.length == 0) {
      ...
      throw new BuildException("Precompiled header prototype: " + prototype.toString()
          + " does not contain any include directives.");
      
    }
    
    final CompilerConfiguration[] configs = new CompilerConfiguration[2];
    
    configs[0] = createPrecompileGeneratingConfig(cmdLineConfig, prototype, includes[0]);
    
    configs[1] = createPrecompileUsingConfig(cmdLineConfig, prototype, includes[0], exceptFiles);
  ...
  protected abstract Parser createParser(File prototype);
