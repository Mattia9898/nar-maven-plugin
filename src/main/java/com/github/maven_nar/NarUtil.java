...
public final class NarUtil {
  ...
  public static String addLibraryPathToEnv(final String path, 
  ...
  static Set<File> findInstallNameToolCandidates(final File[] files, final Log log)
  ...
  public static AOL getAOL(final MavenProject project, final String architecture, final String os, final Linker linker,
      final String aol, final Log log) {
    ...
    * */
    ...
    else
    {
      ...
      aolLinker = getLinkerName(project, architecture, os,linker, log);
      
    }
    
    log.debug("aol_linker: " + aolLinker);

    return aol == null ? new AOL(getArchitecture(architecture), getOS(os), aolLinker) : new AOL(aol);
  ...
  public static File getJavaHome(final File javaHome) {
  ...
  public static String getLinkerName(final MavenProject project, final String architecture, final String os,
      final Linker linker, final Log log) {
  ...
  public static String getOS(final String defaultOs) {
    ...
    if (os == null) {
      ...
      if (name.equalsIgnoreCase("freebsd")) {
	  ...
	  if ( name.equalsIgnoreCase( "SunOS" ) )
	  {
  ...
  public static int runCommand(final String cmd, final String[] args, final String[] env,
      final TextStream err, final TextStream dbg, final boolean expectFailure)
      throws MojoExecutionException, MojoFailureException {
  ...
  static void runInstallNameTool(final File[] files, final Log log) throws MojoExecutionException, MojoFailureException {
    ...
    final Set<File> libs = findInstallNameToolCandidates(files, log);

    for (final File lib1 : libs) {
    	
      final File subjectFile = lib1;
      
      final String subjectPath = subjectFile.getPath();      

      for (final File lib : libs) {
    	  
        final File dependentFile = lib;
  ...
  public static Set<PosixFilePermission> parseOctalPermission(String octal) {
    ...
    if ((dec & 0b000001000) != 0) 
    	
    	permissions.add(PosixFilePermission.GROUP_EXECUTE);
    
    if ((dec & 0b000010000) != 0) 
    	
    	permissions.add(PosixFilePermission.GROUP_READ);
    
    if ((dec & 0b000100000) != 0) 
    	
    	permissions.add(PosixFilePermission.GROUP_READ);
    
    if ((dec & 0b001000000) != 0) 
    	
    	permissions.add(PosixFilePermission.OWNER_EXECUTE);
    
    if ((dec & 0b010000000) != 0) 
    	
    	permissions.add(PosixFilePermission.OWNER_WRITE);
    
    if ((dec & 0b100000000) != 0) 
