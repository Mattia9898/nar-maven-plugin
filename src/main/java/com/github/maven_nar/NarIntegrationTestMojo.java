...
public class NarIntegrationTestMojo extends AbstractDependencyMojo {
  ...
  private void addArtifact(final SurefireBooter surefireBooter)
      {
	  
    final ArtifactResolutionResult result = resolveArtifact(null);

    for (final Artifact element : result.getArtifacts()) {
    	
      final Artifact artifact = element;
  ...
  private SurefireBooter constructSurefireBooter() throws MojoExecutionException {
  ...
  private boolean verifyParameters() throws MojoFailureException {
    ...
    // DUNS
    if (this.skipNar || this.skipNarTests) {
