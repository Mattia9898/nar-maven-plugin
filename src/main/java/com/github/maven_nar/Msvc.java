...
public class Msvc {
  ...
  public void init(final AbstractNarMojo mojo) throws MojoExecutionException {
  ...
  public void configureCCTask(final CCTask task) throws MojoExecutionException {
    ...
    if (mojo != null && OS.WINDOWS.equals(mojo.getOS()) && isMSVC(mojo)) {
      ...
      addIncludePath(msvctoolhome, INCLUDE);
      
      addIncludePath(msvctoolhome, "atlmfc/include");
      ...
      if (compareVersion(windowsSdkVersion, "7.1A") <= 0) {
        ...
        if (version.equals("8.0")) {
          ...
          addIncludePath(windowsSdkHome, INCLUDE);
  ...
  public void configureLinker(final LinkerDef linker) {
  ...
  private boolean addIncludePath(final File base, final String subDirectory)
    ...
    if (file.exists()) {
      ...
      return addIncludePathToTask(file);
  ...
  private boolean addIncludePathToTask(final File file) throws MojoExecutionException {
	  
    try {
    	
      final SystemIncludePath includePath = (SystemIncludePath) createSysIncludePath();
   ...
   */
  public Object createSysIncludePath() {
	  
    return CCTask.compilerDef.createSysIncludePath();
    
  }
  
  private void addWindowsSDKPaths() {
  ...
  private void initVisualStudio() throws MojoExecutionException {
    ...
    if (version.trim().length() > 1) {
      ...
      Pattern r = Pattern.compile("AltraStringaDiTesto");
      ...
      System.getenv("VS" + internalVersion + "COMNTOOLS");   
